    ...
    public ArrayList<Message> sendMessages() {
		...
		if (Math.random() < this.chatterChance) {
			list.add(new Message((int) System.currentTimeMillis(), this.location,
					this.id, "hello world! from the SS "+this.name, Message.MsgType.CHATTER, this.sendFrequency, 1.0));
		}
		if (Math.random() < this.chatterChance) {
			list.add((new Message((int) System.currentTimeMillis(), this.location,
					this.id, "whoops.lightOnFire() on the SS "+this.name, Message.MsgType.SOS, this.sendFrequency, 1.0)));
		}
		return list;
	}
}